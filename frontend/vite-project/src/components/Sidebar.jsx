...
const Sidebar = () => {
  ...
  const fetchWorkspaces = async () => {
    try {
      ...
      const transformedWorkspaces = workspacesData.map((workspace, index) => ({
        ...
        name: workspace.name || workspace.workspace_name || `Workspace ${index + 1}`,
        description: workspace.description || '',
        color: getWorkspaceColor(workspace.name || workspace.id || index),
        role: workspace.role || workspace.user_role || 'member',
        taskCount: workspace.task_count || workspace.taskCount || workspace.tasks?.length || 0,
      ...
      console.log(`Successfully loaded ${transformedWorkspaces.length} workspaces`);
  ...
  const handleWorkspaceClick = (workspace) => {
    ...
    console.log(`Navigating to workspace: ${workspace.name} (ID: ${workspace.id})`);
    window.location.href = `/workspace/${workspace.id}`;
  };

  const handleAddWorkspace = () => {
    console.log('Redirecting to create workspace page...');
  ...
  return (
    <div className={`bg-white border-r border-gray-200 h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-72'}`}>
      {/* Header */}
        ...
        <div className="mt-6 px-3">
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              ...
              {!isCollapsed && <span className="ml-2 text-sm text-gray-500">Loading workspaces...</span>}
            </div>
          )}
            ...
            <div className="space-y-2">
              {workspaces.map((workspace) => (
                <button
                  ...
                  title={isCollapsed ? workspace.name : workspace.description || workspace.name}
                >
                  <div className={`w-3 h-3 rounded-sm ${workspace.color} flex-shrink-0`}></div>
                  {!isCollapsed && (
                    <>
                      <FolderOpen className={`h-4 w-4 ml-3 mr-3 ${
                        activeItem === workspace.id ? 'text-violet-600' : 'text-gray-400'
                      }`} />
              ...
              {workspaces.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400 mb-3">Create your first workspace to get started</p>
                  <button
                    onClick={handleAddWorkspace}
